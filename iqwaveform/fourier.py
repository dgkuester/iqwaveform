...
def _waveform_to_overlap(
    ...
    """stack segments of x into overlapping windows.
...
def upsample(iq: Array, factor: int, Ts: float = None, shift_bins=0, axis=0):
    ...
    pad = [[int(xp.floor(count)) - shift_bins, int(xp.ceil(count)) + shift_bins]]

    X = zero_pad(X, pad + [[0, 0]] * (len(iq.shape) - 1))
    x = ifft(
        xp.fft.fftshift(X, axes=axis),
        axis=axis,
    )
